'use client';
import Image from '@/components/ui/image';
import AvatarIPFS from '@/components/ui/avatar-ipfs';
import Avatar from '@/components/ui/avatar';
import Profile from '@/components/profile/profile';
import UserIMage from '@/assets/images/user.png';
import { selectedProfileAtom } from '@/stores/atoms';
import { useRecoilState } from 'recoil';
// static data
import { authorData } from '@/data/static/author';
import { ProfileContractContext } from '@/contracts-context/ProfileContractContext';
import { useContext, useEffect, useState } from 'react'; // Import useEffect and useState

// import { readIPFS } from '@/app/shared/ipfs';
import { User } from '@/app/shared/types';
const AuthorProfilePage = () => {
  const {
    profileData,
    setcurrentProfileSelected,
    allProfiles,
    setAllProfiles,
  } = useContext(ProfileContractContext);

  const [selectedProfile, setSelectedProfile] =
    useRecoilState(selectedProfileAtom);
  useEffect(() => {
    //get index of profile where isOrganisation is false
    const index = allProfiles.findIndex(
      (profile) => profile.value.isOrganisation === false,
    );
    //set currentProfileSelected to the profile where isOrganisation is false
    setcurrentProfileSelected(allProfiles[index]);
    setSelectedProfile(allProfiles[index]);
    //update allProfiles to set selected to true for the profile where isOrganisation is false
    setAllProfiles((prev) => {
      return prev.map((profile, i) => {
        if (i === index) {
          return { ...profile, selected: true };
        } else {
          return { ...profile, selected: false };
        }
      });
    });
  }, []);
  return (
    <>
      <div className="relative h-36 w-full overflow-hidden rounded-lg sm:h-44 md:h-64 xl:h-80 2xl:h-96 3xl:h-[448px]">
        <Image
          src={authorData?.cover_image?.thumbnail}
          placeholder="blur"
          quality={100}
          className="!h-full w-full !object-cover"
          alt="Cover Image"
        />
      </div>
      <div className="mx-auto flex w-full shrink-0 flex-col md:px-4 xl:px-6 3xl:max-w-[1700px] 3xl:px-12">
        {profileData?.profilePhotoHash &&
          profileData?.profilePhotoHash.size != 0 && (
            <AvatarIPFS
              hash={profileData?.profilePhotoHash}
              size="xl"
              alt="Author"
              className="z-10 mx-auto -mt-12 dark:border-gray-500 sm:-mt-14 md:mx-0 md:-mt-16 xl:mx-0 3xl:-mt-20"
            />
          )}
        {profileData?.profilePhotoHash &&
          profileData?.profilePhotoHash.size == 0 && (
            <Avatar
              size="xl"
              image={UserIMage}
              alt="Author"
              className="z-10 mx-auto -mt-12 dark:border-gray-500 sm:-mt-14 md:mx-0 md:-mt-16 xl:mx-0 3xl:-mt-20"
            />
          )}
        <Profile />
      </div>
    </>
  );
};

export default AuthorProfilePage;
