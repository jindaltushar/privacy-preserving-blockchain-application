...
export function AnswerNode({
  ...
  const setAns = (ans: string) => {
    setNodes((prev) => {
      const elem = prev[i] as CheckboxProps;
      const optionOptions = [
  ...
  const addAnswerNode = () => {
    ...
    const elem = nodes[i] as CheckboxProps;
    ...
    <div className="flex items-center justify-between gap-2">
      ...
      <div className="flex-grow">
        <Input
          ...
          value={(nodes[i] as CheckboxProps).data.optionOptions[ai]}
...
export function CheckboxNode({ index }: { index: number }) {
  const [i] = useState<number>(index);
  const [nodes, setNodes] = useRecoilState(nodesAtom);
    ...
    try {
      ...
      if (qn.length > 3) {
        if (!masterSettings.is_survey_private) {
          ...
          var res = await GetMatchingQuestions(qn, 1);
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        {!masterSettings.is_survey_private && (
          <OptionNode
            ...
            nodes={nodes as CheckboxProps[]}
            setNodes={setNodes}
            fetchedOptions={fetchedOptions}
          />
        )}

        {(nodes[i] as CheckboxProps).data.ans.map((value, index) => {
          ...
          return (
            <OptionNode
              ...
              optionsList={fetchedOptions.filter(
                ({ index }) =>
                  !(nodes[i] as CheckboxProps).data.ans.includes(index),
              )}
              nodes={nodes as CheckboxProps[]}
              setNodes={setNodes}
              fetchedOptions={fetchedOptions}
        ...
        {(nodes[i] as CheckboxProps).data.other && (
        ...
        {(nodes[i] as CheckboxProps).data.other &&
          (nodes[i] as CheckboxProps).data.optionOptions.map((value, index) => {
