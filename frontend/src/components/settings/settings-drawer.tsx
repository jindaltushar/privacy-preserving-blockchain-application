...
import { preferedCurrencyAtom } from '@/stores/atoms';
import { useRecoilState } from 'recoil';
import IconEuro from '@/components/icons/euro';
import IconCurrencyUsd from '@/components/icons/usd';
const ColorPreset = [
  {
    label: 'Black',
    value: '#323743',
...
// Component: PreferedCurrencySwitcher
function PreferedCurrencySwitcher() {
  const [preferedCurrency, setPreferedCurrency] =
    useRecoilState(preferedCurrencyAtom);
  return (
    <div className="px-6 pt-8">
      <h4 className="mb-4 text-sm font-medium text-gray-900 dark:text-white">
        Currency
      ...
      <RadioGroup
        ...
        onChange={setPreferedCurrency}
        className="grid grid-cols-2 gap-5 "
      >
        <RadioGroup.Option value="USD">
          {({ checked }) => (
            <SwitcherButton title={'USD'} checked={checked}>
              <IconCurrencyUsd height="25px" width="25px" />
            </SwitcherButton>
          )}
        </RadioGroup.Option>
        <RadioGroup.Option value="EUR">
          {({ checked }) => (
            <SwitcherButton title={'EUR'} checked={checked}>
              <IconEuro height="25px" width="25px" />
            </SwitcherButton>
          )}
        </RadioGroup.Option>
      </RadioGroup>
